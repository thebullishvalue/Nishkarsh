...
def _composite_signal(frame: pd.DataFrame, w: dict[str, float]) -> np.ndarray:
    """Recompute the signed convergence composite from per-dim scores.

    Mirrors cross_validator.py's formula:
        composite = Σ wₖ · (2·score_k − 1)  ∈ [−1, +1]
    Then scaled to [−100, +100] to match the legacy convergence_score scale.
    """
    ...
    return (M @ weights) * 2.0 - weights.sum()  # = M @ w * 2 - 1 (weights sum to 1)
...
class ConvergenceTuner:
    ...
    def __init__(
        ...
        # Chronological split — no shuffling, preserves causality
        n = len(frame)
        n_train = max(20, int(n * self.train_frac))
        self.train_frame = frame.iloc[:n_train].copy()
        self.val_frame   = frame.iloc[n_train:].copy()
    ...
    def _objective(self, trial: "optuna.Trial") -> float:
        ...
        _, score = _score_frame(self.train_frame, w, t, self.horizons)
        if np.isnan(score):
            return -1e6
