"""
Nishkarsh — Precedent view (historical analog matching + forward returns + backtest).

Ports Arthagati's Similar-Periods view (inherited from Tattva): covariance-aware
Mahalanobis analog cards, a forward-return base-rate summary, and a descriptive
state→forward-return backtest. Inputs are Nishkarsh's engine.ts_data state features;
forward-return horizons follow the Aarambh forecast lens (AARAMBH_FWD_*).
"""
...
def render_precedent_tab(
    ...
    render_interpretation_card(
        ...
        body=(
            ...
            "on Nishkarsh's own state features). Read it <strong>alongside</strong> the "
